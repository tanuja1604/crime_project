...
with col1:
    ...
    rpt_dist = st.number_input("Report District No", min_value=1, value=100)

    part = st.selectbox("Part 1-2",[1,2])

    crm_cd = st.number_input("Crime Code", value=110)

    victim_age = st.slider("Victim Age",0,100,25)
...
with col2:
    ...
    premis_cd = st.number_input("Premise Code", value=101)
    ...
    weapon_cd = st.number_input("Weapon Used Code", value=0)
...
if victim_age >=18 and victim_age<30:
    age_young=1

elif victim_age>=30 and victim_age<50:
    age_adult=1
...
if st.button("Predict Crime"):
    ...
    crime = target_encoder.inverse_transform(prediction)

    st.success(f"Predicted Crime Category : **{crime[0]}**")